...
# 	np.save('preprocessed_finaltest_iitmdata',[x_train,y_train,dtw_train,[0]])

# preprocess(data)
# [x_train,y_train,dtw_train,temp] = np.load('preprocessed_finaltest_iitmdata.npy')
...
# np.save('preprocessed_finaltest_iitmdata_final',[x_train,y_train,dtw_train,[0]])
[x_train,y_train,dtw_train,temp] = np.load('preprocessed_finaltest_iitmdata_final.npy')
print('yippeee')
...
for iterationnum in range(n_fold):
	...
	with tf.Session() as sess:
		...
		# np.save('ds_finaltest'+str(iterationnum)+'.npy',[d1_test,d2_test,d3_test,d4_test])
		[d1_test,d2_test,d3_test,d4_test] = np.load('ds_finaltest'+str(iterationnum)+'.npy')
		...
		# print(logits_test)
		pred = softmax(logits_test,axis=1)
		argmax_prediction = np.argmax(pred,axis=1)
		print(argmax_prediction)
		predictions.append(argmax_prediction)
		# print("F1-test:", f1(logits_test,y_test))
